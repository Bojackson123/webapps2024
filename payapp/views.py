        ...
        try:
            ...
            if receiver and request.user.balance >= amount and amount > 0:
                with db_transaction.atomic():
                    ...
                    request.user.save()
                    receiver.save()
                    message=f"You've received {round(converted_amount, 2)} {receiver.currency} from User: {request.user.username}"
                    balance = float(receiver.balance)
                    sign = receiver.get_currency_sign()
                    print(balance, sign)
                    send_notification(receiver, message, '/webapps2024/?tab=re_payments', balance,
                                      sign)
...
def request_payment(request):
    if request.method == "POST":
        ...
        if receiver and amount > 0 and receiver != request.user:
            ...
            tr.save()
            message = f"Payment Request of {round(converted_amount, 2)} {receiver.currency} from User: {request.user.username}"
            balance = float(receiver.balance)
            sign = receiver.get_currency_sign()
            send_notification(receiver, message, '/webapps2024/?tab=re_request_payments',
                              balance, sign)
...
def accept_payment_request(request, request_id):
    ...
    if transaction:
        with db_transaction.atomic():
            ...
            transaction.save()
            message = f"User: {transaction.sender.username} Accepted your payment request of {transaction.amount} {transaction.receiver.currency}."
            balance = float(transaction.receiver.balance)
            sign = transaction.receiver.get_currency_sign()
            send_notification(transaction.receiver, message, '/webapps2024/?tab=sent_request_payments',
                              balance, sign)
...
def reject_payment_request(request, request_id):
    ...
    if transaction:
        ...
        transaction.save()
        message = f"User: {transaction.sender.username} Rejected your payment request of {transaction.amount} {transaction.receiver.currency}."
        balance = float(transaction.receiver.balance)
        sign = transaction.receiver.get_currency_sign()
        send_notification(transaction.receiver, message, '/webapps2024/?tab=sent_request_payments',
                          balance, sign)
